...
########################################################################
#                                                                      #
#           Instalaçao do Pterodactyl ArthurZiiin                      #
#            Copyright 2023 Pterodactyl ArthurZiiin                    #
#                                                                      #
########################################################################
...
finish(){
    ...
    echo "[!] Instalação do Painel Pterodáctilo concluída"
    echo ""
    ...
    echo " panel_credentials.txt no diretório atual"
    echo ""
    echo " Você gostaria de instalar o Wings também? (S/N)"
...
panel_webserver(){
    send_summary
    echo "[!] Selecione o Sistema do Site"
    echo "    (1) NGINX"
    echo "    (2) Apache"
    echo "    Input 1-2"
    read -r option
    case $option in
        ...
        2 ) option=2
            ...
            ;;
        * ) echo ""
            echo "Obrigatorio colcoar 1 ou 2 :)"
    esac
}
...
panel_fqdn(){
    send_summary
    echo "[!] Coloque o seu Dominio ou ip Numerico"
    read -r FQDN
    ...
    if [ "${IP}" != "${DOMAIN}" ]; then
        ...
        echo "Continuando mesmo assim em 10 segundos... Pressione CTRL+C para interromper."
        sleep 10s
        panel_ssl
    else
        panel_ssl
...
panel_username(){
    send_summary
    echo "[!] Coloque o Usuario do Painel."
    read -r USERNAME
    panel_firstname
}
panel_firstname(){
    send_summary
    echo "[!] Segundo nome do Painel."
    read -r FIRSTNAME
    panel_lastname
}
panel_lastname(){
    send_summary
    echo "[!] Ultimo nome do Painel."
    read -r LASTNAME
    panel_summary
}
...
wings(){
    ...
    echo ""
    echo "[!] Informaçoes da Instalaçao"
    echo ""
    wings_fqdn
}

wings_fqdnask(){
    echo "[!] Quer instalar Certificado? (Y/N)"
    echo "    Coloque seu Email"
    echo "    o email vai ficar cravado no SSL."
    read -r WINGS_SSL
...
wings_full(){
    if  [ "$WINGS_FQDN_STATUS" =  "true" ]; then
        ...
        clear
        echo ""
        echo "[!] Asas do Pterodactyl Instaladas com Exito."
        echo "    Renicie a maquina e vai estar online"
        echo ""
        fi
...
wings_fqdn(){
    echo "[!] Coloque seu Dominio ou ip Numerico"
    read -r WINGS_FQDN
    IP=$(dig +short myip.opendns.com @resolver2.opendns.com -4)
...
options(){
    echo "ArthurZiin pede pra escolher uma Opçao?"
    echo "[1] Instalar Pterodactyl."
    echo "[2] Instalar Asas."
    echo "[3] Instalar PHPMyAdmin."
    echo "[4] Remover Asas"
    echo "[5] Remover Painel"
    echo "[6] Mudar o Dominio do Painel"
    echo "Input 0-6"
    read -r option
    case $option in
        1 ) option=1
...
clear
echo ""
echo "Pterodactyl Instalaçao @ v2.0"
echo "Copyright 2023, ArthurZiiin"
echo "https://github.com/ArthurZiiin/Pterodactyl"
echo ""
echo "Esse Script nao e oficial e nem ta 100%"
echo ""
oscheck
